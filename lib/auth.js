...
export const authInitialProps = isProtectedRoute => ({req, res}) => {
  ...
  // console.log(req.headers)
    ...
    if(serverToken){
      serverToken = JSON.parse(serverToken);
      // console.log('currentToken',serverToken)
    }
  }
  
  const auth = req ? serverToken : getClientSideToken();
  const currentPath = req ? req.url : window.location.pathname; // checking where the req is coming from
  const user = req? serverToken : auth.user;
  // console.log(user)
  // console.log(user.type)
  // console.log(serverToken.type)
const isAnonymous = !user || user.type !== 'authenticated';

  if (isProtectedRoute && isAnonymous && currentPath !== "/login"){
    console.log('ya hhochu redirect potomu chto eto true', isProtectedRoute, isAnonymous)
